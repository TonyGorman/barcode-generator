...
test.describe('Label Generator regressions - visual baselines', () => {
  ...
  test('captures Mini SEL three-row aisle preview baseline', async ({ page }) => {
    await page.setViewportSize({ width: 1800, height: 1400 })
    await page.goto('/')
    await selectMiniVariant(page, 'mini-three-row')
    ...
    const previewPage = page.locator('[class*="previewPage"]').first()
    await expect(previewPage).toHaveScreenshot('mini-sel-three-row-aisle-preview.png', {
      animations: 'disabled',
      maxDiffPixelRatio: 0.05,
    })
  })
