...
describe('labelGeneration', () => {
  ...
  it('validates short label bounds and generates compact output', () => {
    ...
    expect(generateShortLabelCodes(shortInput, formatTwoDigitValue)).toEqual(['BAK01A', 'BAK01B', 'BAK02A', 'BAK02B'])
  })
  ...
  it('validates short shelf ordering and supports shelf ranges from custom starts', () => {
    ...
    expect(generateShortLabelCodes(validRangeInput, formatTwoDigitValue)).toEqual(['BAK01B', 'BAK01C'])
  })

  it('normalizes specific input tokens to compact uppercase values', () => {
    expect(normalizeSpecificInputCodes(' 01l01a, bak01a , , kiosk ')).toEqual(['01L01A', 'BAK01A', 'KIOSK'])
  })
