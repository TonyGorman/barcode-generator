...
describe('SpecificLabelForm', () => {
  ...
  it('accepts configured prefixed aisle values in compact form', () => {
    ...
    expect(screen.queryByRole('alert')).not.toBeInTheDocument()
    expect(screen.getByTestId('generated-labels')).toHaveTextContent(
      `${AISLE_PREFIXES[0]}10L01A|${AISLE_PREFIXES[1]}2L02B`,
    )
  })
  ...
  it('rejects separated short code input (dashes not allowed)', () => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent(
      `Label '${SHORT_CODE_PREFIXES[0]} 01 A' must not contain spaces or dashes`,
    )
  })
  ...
  it('rejects separated front-of-store values', () => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent(
      `Label '${SHORT_CODE_PREFIXES[1]}-01-A' must not contain spaces or dashes`,
    )
  })
  ...
  it('accepts both Back and Front wall values in the same submission', () => {
    ...
    expect(screen.queryByRole('alert')).not.toBeInTheDocument()
    expect(screen.getByTestId('generated-labels')).toHaveTextContent(
      `${SHORT_CODE_PREFIXES[0]}01A|${SHORT_CODE_PREFIXES[1]}01A`,
    )
  })
