...
describe('evaluateLabelBatchLimits', () => {
  it('returns no warning or error when count is within limits', () => {
    expect(evaluateLabelBatchLimits(10, 100, 200)).toEqual({
      hardLimitError: null,
      warningMessage: null,
    });
  });
  ...
  it('returns warning when count exceeds soft limit but not hard limit', () => {
    const result = evaluateLabelBatchLimits(101, 100, 200);
  ...
  it('returns hard limit error and no warning when count exceeds hard limit', () => {
    const result = evaluateLabelBatchLimits(201, 100, 200);
